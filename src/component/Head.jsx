...
export default function Head() {
    ...
    useEffect(() => {
        // Create a custom loading manager that blocks texture loading
        const customManager = new THREE.LoadingManager();
        const originalGetHandler = customManager.getHandler;
        
        customManager.getHandler = function(url) {
            // Block all texture file requests
            if (url.match(/\.(png|jpg|jpeg|gif|bmp|tga|webp)$/i)) {
                console.log('Blocked texture loading:', url);
                return {
                    load: function(url, onLoad, onProgress, onError) {
                        // Create a dummy texture to prevent errors
            ...
            // For other files, use the original handler
            return originalGetHandler.call(this, url);
        };

        const loader = new FBXLoader(customManager)
